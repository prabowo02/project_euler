...
int main () {
    ...
    while (n--) {
        gets(s);
        FORR(i, 50) res[49-i] += s[i] - '0';
    }

    FORR(i, 49) res[i + 1] += res[i] / 10, res[i] %= 10;
    while (res.back() > 9) res.pb(res.back() / 10), res[res.size()-2] %= 10;

    for (int i=0; i<10; i++) printf("%d", res.back()), res.pop_back();
    printf("\n");
